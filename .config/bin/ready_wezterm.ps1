function LaunchPanes {
  ...
  foreach ($paneName in $paneNames) {
    switch ($paneName) {
      ...
      "RENDER" {
        # Build the folder path where renderConfigBlender.py is located
        $renderFolder = Split-Path $renderFull -Parent
        
        # Define the additional files in the same folder
        $sceneConfigBlender = Join-Path $renderFolder "sceneConfigBlender.psd1"
        $sceneConfigAfterFX = Join-Path $renderFolder "sceneConfigAfterFX.psd1"
        $run = Join-Path $renderFolder "run.ps1"
        
        # Spawn pane running Neovim with all three files
        wezterm cli spawn --cwd "$projectRoot" -- nvim "$renderFull" "$sceneConfigAfterFX" "$sceneConfigBlender" "$run"
      }
      "DIRECTORY" {
